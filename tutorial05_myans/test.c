...
#if defined(_MSC_VER)  // _MSC_VER 是MSVC编译器的宏,定义了编译器的版本
...
static void test_parse_array() {
    ...
    for (size_t i = 0; i < 4; i++) {
        // 2.1 取出子数组，看各元素的格式是否正确(数组元素格式合乎文法)
        const lept_value* e = lept_get_array_element(&v, i);//获取一行
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(e));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(e));
        for (size_t j = 0; j < i; j++) {
            // 3. 看各元素的值是否正确(数组元素的值正确)
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(lept_get_array_element(e, j)));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(lept_get_array_element(e, j)));
        }
    }
    lept_free(&v);
}
