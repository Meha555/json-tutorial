...
#define BETWEEN(x, left, right) ((x) >= (left) && (x) <= (right))

static const char* lept_parse_hex4(const char* p, unsigned* u) {
    *u = 0;
    for (int i = 0; i < 4;i++){//处理1个字节
        char ch = *p++;
        *u <<= 4;//左移4位
        if(BETWEEN(ch,'0','9')) *u|=ch-'0';//赋予第一个十六进制的值
        else if (BETWEEN(ch,'A','F')) *u|=ch-'A'+10;
        else if (BETWEEN(ch,'a','f')) *u|=ch-'a'+10;
        else return NULL;//检测NULL判断是否失败
    }
    return p;
}
        ...
        switch (ch) {
            ...
            case '\\':
                switch (*p++) {
                    ...
                    case 't':  PUTC(c, '\t'); break;
                    case 'u'://是utf-8转义字符
                        if (!(p = lept_parse_hex4(p, &u)))//将得到的UTF-8码点存到u中
